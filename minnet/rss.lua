...
rss = {
    ...
    feeds = {       -- Network names _must_ be lowercase
        illumine = {
            ...
            {
            name = "Minnet-git",
            url  = "https://github.com/staeld/minnet/commits/dev.atom",
            out  = "",
            patt = "",
            chan = { "#valo" },
            freq = "60m",
...
-- We use wget for the sake of laziness and simplicity: abort if not found
if io.popen("which wget"):read("*a") == "" then
    log("wget not found", "error")
end
    ...
    for i, f in ipairs(rss.feeds[net.name:lower()]) do
        ...
        if ( not f.updated or os.difftime(os.time(), f.updated) >= length )
          or not rss.has_feed(f.name) then
            ...
            rss.save_times()
            rss.read_latest(f.name)         -- Check for new entries and report
        else
